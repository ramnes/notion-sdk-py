...
class TestIdExtractionUtilities:
    ...
    def test_extract_notion_id_with_malformed_url(self):
    ...
    def test_extract_notion_id_with_formatted_uuid_in_query(self):
        """Test extracting already formatted UUID from query parameters."""
        url = "https://notion.so/share?p=12345678-1234-1234-1234-123456789abc"
        expected = "12345678-1234-1234-1234-123456789abc"
        assert extract_notion_id(url) == expected

    def test_extract_notion_id_with_fragment_fallback(self):
        """Test fragment extraction in general function."""
        url = "https://www.notion.so/Page#abc123def456789012345678901234ab"
        expected = "abc123de-f456-7890-1234-5678901234ab"
        assert extract_notion_id(url) == expected

    def test_format_uuid_error_handling(self):
    ...
    def test_extract_block_id_exception_handling(self):
    ...
    def test_extract_block_id_with_formatted_uuid_fragment(self):
        """Test extract_block_id with already formatted UUID in fragment."""
        url = "https://www.notion.so/Page#12345678-1234-1234-1234-123456789abc"
        expected = "12345678-1234-1234-1234-123456789abc"
        assert extract_block_id(url) == expected

    def test_extract_notion_id_empty_path(self):
        """Test URL with empty or no path."""
        url = "https://notion.so/?p=abc123def456789012345678901234ab"
        expected = "abc123de-f456-7890-1234-5678901234ab"
        assert extract_notion_id(url) == expected

    def test_extract_notion_id_empty_query_params(self):
        """Test URL with empty query parameter values."""
        url = "https://notion.so/Page?p=&id="
        assert extract_notion_id(url) is None

    def test_extract_notion_id_invalid_uuid_length_in_query(self):
        """Test query parameter with invalid UUID length."""
        url = "https://notion.so/Page?p=tooshort"
        assert extract_notion_id(url) is None

    def test_extract_notion_id_non_string_input(self):
        """Test with non-string input types."""
        inputs = [123, [], {}, set()]
        for invalid_input in inputs:
            assert extract_notion_id(invalid_input) is None

    def test_extract_notion_id_fragment_without_compact_match(self):
        """Test fragment that doesn't match compact UUID pattern."""
        url = "https://www.notion.so/Page#not-a-valid-uuid-fragment"
        assert extract_notion_id(url) is None

    def test_extract_block_id_non_url_fallback(self):
        """Test extract_block_id fallback for non-URL strings."""
        # This should fall back to extract_notion_id
        compact_uuid = "123456781234123412341234567890ab"
        expected = "12345678-1234-1234-1234-1234567890ab"
        assert extract_block_id(compact_uuid) == expected

    def test_extract_block_id_url_without_protocol(self):
        """Test extract_block_id with string that doesn't contain ://"""
        result = extract_block_id("not-a-url-string")
        assert extract_notion_id("not-a-url-string") == result

    def test_extract_notion_id_with_block_prefix_in_fragment(self):
    ...
    def test_extract_block_id_with_non_string_input(self):
        """Test extract_block_id with non-string input (line 260)."""
        assert extract_block_id(None) is None
        assert extract_block_id(123) is None
        assert extract_block_id([]) is None

    def test_extract_notion_id_url_parsing_exception(self):
